export async function fetch_Music() {
  const result = await fetch("https://music-shop-backend-jwdn.onrender.com/all").then((response) =>
    response.json()
  );
...
export async function delete_Music(Id) {
  const result = await fetch(`https://music-shop-backend-jwdn.onrender.com/delete?Id=${Id}`, {
    method: "DELETE"
